...
//
// Every fallback is info@firstkeyint.com deliberately. It is the only mailbox
// that exists — the Hostinger free plan includes one account, so careers@ and
// vishal@ do not resolve. Defaulting to them meant a missing env var routed
// mail to a dead address: the SMTP relay still accepted the message, the API
// still returned 200, and the submission was lost with no visible error.
// Environment variables have already gone missing once, during the 2026-07-31
// Vercel account migration. Keep these fallbacks pointed at a real mailbox.
const DEFAULT_INBOX = "info@firstkeyint.com";

export const INBOXES = {
  listing: process.env.LISTINGS_TO || DEFAULT_INBOX,
  careers: process.env.CAREERS_TO || DEFAULT_INBOX,
  contact: process.env.CONTACT_TO || DEFAULT_INBOX,
};
